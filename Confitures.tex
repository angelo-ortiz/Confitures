...
\'Etant donn\'e que les suites $b(s)$ et $c(s)$ sont croissantes\todo{pourquoi ?}, les \'equations (1) et (2) deviennent
\begin{alignat*}{1}
...
Donc, la complexit\'e pour le syst\`eme de 2 capacit\'es minimales est donn\'ee par $a(s)$. Ainsi, la complexit\'e est exponentielle en $S$, la quantit\'e de confiture \`a disposition. \todo{quid du cas g\'en\'eral ?}
...
\begin{algorithmic}[1]
\Require $k \geq 0$ \AND $S \geq 0$
\Function{ConfitureBackwards}{$k$: integer, $V$: $k$-integer array, $S$: integer, $M$: $S \times k$ integer matrix}
