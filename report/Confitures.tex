...
\EndFunction
\Statex
\Function{SetMsi}{$V$: $k$-integer array, $M$: $(S+1) \times (k+1)$ integer array matrix, $s$: integer, $i$: integer}
...
L'algorithme \texttt{AlgoGlouton} consiste en une boucle contenant seulement quatre instruction \'el\'ementaires. De plus, on effectue au plus $k$ tours de boucle car, pour la $k$-i\`eme it\'eration, on utilise de bocaux de capacit\'e 1 \si{dg} et on peut ainsi verser toute la confiture restante dans des bocaux de cette capacit\'e. De ces faits, la complexit\'e temporelle de l'algorithme \texttt{AlgoGlouton} dans le pire des cas est en $\BigO(k)$.
...
\begin{enumerate}[a)]
 ...
 \end{equation*}
  ...
  De ces faits, \( \displaystyle\sum_{i=1}^{k} V[i]\cdot o_i = \displaystyle\sum_{i=1}^{k} V[i]\cdot o_i'\), i.e.\ $o'$ est aussi une solution du probl\`eme. \todo{d\'eduire que Expo est glouton-compatible}
\end{enumerate}
