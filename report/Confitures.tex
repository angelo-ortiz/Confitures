...
\begin{enumerate}[a)]
 ...
 Voici le pseudocode des modifications effectu\'ees :
...
\subsection*{Question 6}
 ...
 De ce fait, on peut dire que cet algorithme est polynomial.
...
\begin{enumerate}[a)]
 ...
 En soustrayant \eqref{eq:somme_9b2} de \eqref{eq:somme_9b1}, on obtient \( \displaystyle\sum_{i=1}^{j} V[i]\cdot g_i = \displaystyle\sum_{i=1}^{j} V[i]\cdot o_i\).
 ...
 \end{equation*}
  ...
  \end{equation*}
  i.e.\ $o'$ est aussi une solution optimale du probl\`eme. 
  
  \medskip
  On remarque que la diff\'erence entre $o'$ et $o$ est que la premi\`ere remplace $d$ bocaux d'une certaine capacit\'e dans la deuxi\`eme solution par un bocal de la capacit\'e imm\'ediatement sup\'erieure. En r\'ep\'etant ce proc\'ed\'e un nombre fini de fois, on remplace des bocaux de petite capacit\'e par ceux de plus grande capacit\'e de sorte \`a utiliser, pour chaque plus grande capacit\'e s\'electionnable, le plus possible de bocaux. Cette m\'ethode correspondant \`a la solution $g$, on a que la solution gloutonne est optimale. On en d\'eduit que le syst\`eme {\bfseries Expo} est glouton-compatible.
\end{enumerate}
...
\subsection*{Question 10}
 ...
 Montrons que la complexit\'e de l'algorithme \texttt{TestGloutonCompatible}$(k, V)$ est en $\BigO(k)$, i.e.\ qu'il est polynomial.

 \subsubsection*{Preuve}
 Il y a $nbiter_ext = V[k-1]+V[k]-1 -(V[3]+2) +1 $ it\'erations de la boucle externe.
 Or $V[3]+2 \geq 5$ et $V[k-1]+V[k]-1 \leq 2\cdot V[k] - 2$. Donc, $nbiter_ext \leq 2\cdot V[k] - 6$, i.e. le nombre d'it\'erations de la boucle externe est en $\Theta(V[k])$.
 
 Par ailleurs, on effectue le m\^eme nombre d'it\'erations de la boucle interne pour chaque tour de la boucle externe, \`a savoir $k$. De plus, chaque tour de la boucle interne comporte un premier test de v\'erit\'e entre deux nombres obtenus en $\Theta(1)$ \`a savoir $V[j]$ et $S$, et un deuxi\`eme test sur deux nombres obtenus chacun au moyen d'un appel \`a \texttt{AlgoGlouton}. D'apr\`es la question 7, sa complexit\`e est en $\BigO(k)$. Donc, chaque tour de la boucle interne est fait en $\BigO(k)$.
 
 De ces faits, la complexit\`e de l'algorithme \texttt{TestGloutonCompatible} est en $\BigO(V[k]\cdot k^2)$. On en d\'eduit que l'algorithme est polynomial.
 \todo{montrer}
